...
with col1:
    ...
    # Location dropdown (matches backend enum values)
    st.markdown("<p><strong>Location</strong></p>", unsafe_allow_html=True)
    location = st.selectbox("", options=["Downtown", "Suburb", "Rural"], index=1, label_visibility="collapsed")

    # Condition dropdown (matches backend enum values)
    st.markdown("<p><strong>Condition</strong></p>", unsafe_allow_html=True)
    condition = st.selectbox("", options=["Excellent", "Good", "Fair", "Poor"], index=1, label_visibility="collapsed")
...
with col2:
    ...
    if "prediction" in st.session_state:
        ...
        confidence_interval = pred.get("confidence_interval", [0.0, 0.0])
        feature_importance = pred.get("features_importance", pred.get("feautures_importance", {}))
        prediction_time_text = pred.get("prediction_time", pred.get("predcition_time", "N/A"))
        ...
        with col_c:
            ...
            lower = "${:,.1f}".format(confidence_interval[0])
            upper = "${:,.1f}".format(confidence_interval[1])
        ...
        with col_d:
            ...
            st.markdown(f'<p class="info-value">{prediction_time_text}</p>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
        ...
        if feature_importance:
            factors = "".join([f"<li>{name}</li>" for name in list(feature_importance.keys())[:3]])
            st.markdown(f"<ul>{factors}</ul>", unsafe_allow_html=True)
        else:
            st.markdown("""
            <ul>
